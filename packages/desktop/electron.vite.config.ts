...
// Copy the Infinite Canvas sub-app (packages/desktop/resources/canvas) into
// out/renderer/canvas right after the renderer bundle is emitted. electron-vite
// empties out/renderer during the build, so the copy must run in the renderer
// (last) closeBundle. The web-host static server serves out/renderer at
// http://127.0.0.1:25808, exposing the canvas at /canvas/index.html.
function copyCanvasPlugin() {
  return {
    name: 'vite-plugin-copy-canvas',
    closeBundle() {
      execSync(`node "${resolve('scripts/copy-canvas.js')}"`, { stdio: 'inherit' });
    },
  };
}
