...
import { SiderToolbar, SiderSearchEntry, SiderScheduledEntry, SiderAssistantEntry, SiderCanvasEntry } from './SiderNav';
...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  const handleCanvasClick = () => {
    ...
    closePreview();
    setIsBatchMode(false);
    Promise.resolve(navigate('/canvas')).catch((error) => {
      console.error('Navigation failed:', error);
    });
    if (onSessionClick) {
      onSessionClick();
      ...
      <div className='flex-1 min-h-0 overflow-hidden'>
        {isSettings ? (
          ...
          <div className='size-full flex flex-col gap-2px'>
            ...
            />
            {/* Infinite Canvas nav entry - fixed above scroll */}
            <SiderCanvasEntry
              isMobile={isMobile}
